...
DEFINE_LOG_CATEGORY_STATIC(LogRealGazeboBridgeManager, Log, All);

ARealGazeboBridgeManager::ARealGazeboBridgeManager()
{
    PrimaryActorTick.bCanEverTick = true;
    PrimaryActorTick.TickInterval = 1.0f; // Update status every second
}

void ARealGazeboBridgeManager::BeginPlay()
{
    Super::BeginPlay();
    ...
    {
        UE_LOG(LogRealGazeboBridgeManager, Warning, TEXT("Configuration validation failed. Bridge will not start automatically."));
        return;
    }
    ...
    GetWorld()->GetTimerManager().SetTimer(StatusUpdateTimer, this, &ARealGazeboBridgeManager::UpdateStatusDisplay, 1.0f, true);
}

void ARealGazeboBridgeManager::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
    // Clean up timer
    if (GetWorld())
    {
...
void ARealGazeboBridgeManager::Tick(float DeltaTime)
{
...
#if WITH_EDITOR
void ARealGazeboBridgeManager::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
...
void ARealGazeboBridgeManager::StartBridge()
{
    if (!BridgeSubsystem.IsValid())
    {
    ...
    if (!ValidateConfiguration())
    {
        UE_LOG(LogRealGazeboBridgeManager, Error, TEXT("Cannot start bridge - invalid configuration"));
        return;
    }
    ...
    UE_LOG(LogRealGazeboBridgeManager, Log, TEXT("RealGazebo Bridge started - Port: %d, IP: %s"), ListenPort, *ServerIPAddress);
...
void ARealGazeboBridgeManager::StopBridge()
{
    if (!BridgeSubsystem.IsValid())
    {
        return;
    ...
    UE_LOG(LogRealGazeboBridgeManager, Log, TEXT("RealGazebo Bridge stopped"));
...
bool ARealGazeboBridgeManager::IsBridgeActive() const
{
    return BridgeSubsystem.IsValid() && BridgeSubsystem->IsBridgeActive();
}

void ARealGazeboBridgeManager::ClearAllVehicles()
{
    if (BridgeSubsystem.IsValid())
    {
        BridgeSubsystem->ClearAllVehicles();
        UE_LOG(LogRealGazeboBridgeManager, Log, TEXT("All vehicles cleared"));
    }
}

int32 ARealGazeboBridgeManager::GetActiveVehicleCount() const
{
    if (BridgeSubsystem.IsValid())
    {
...
void ARealGazeboBridgeManager::GetNetworkStatistics(int32& OutValidPackets, int32& OutInvalidPackets, float& OutPacketsPerSecond) const
{
    OutValidPackets = 0;
    OutInvalidPackets = 0;
...
void ARealGazeboBridgeManager::ConfigureSubsystem()
{
    if (!BridgeSubsystem.IsValid())
    {
        return;
    ...
    UE_LOG(LogRealGazeboBridgeManager, Log, TEXT("Subsystem configured - Port: %d, IP: %s, DataTable: %s"), 
           ListenPort, 
           *ServerIPAddress, 
           VehicleDataTable ? *VehicleDataTable->GetName() : TEXT("None"));
}

void ARealGazeboBridgeManager::ConfigureVehiclePoolSettings()
{
    if (!BridgeSubsystem.IsValid())
    {
        return;
    ...
    if (UVehiclePoolManager* PoolManager = BridgeSubsystem->GetVehiclePoolManager())
    {
        ...
        UE_LOG(LogRealGazeboBridgeManager, Verbose, TEXT("Pool settings configured - MaxPerType: %d, InitialSize: %d"), 
               MaxActorsPerType, InitialPoolSize);
    }
}

void ARealGazeboBridgeManager::ConfigureNetworkProcessingSettings()
{
    if (!BridgeSubsystem.IsValid())
    {
        return;
    ...
    if (UDataStreamProcessor* StreamProcessor = BridgeSubsystem->GetDataStreamProcessor())
    {
        ...
        UE_LOG(LogRealGazeboBridgeManager, Verbose, TEXT("Network processing configured - BatchSize: %d, BatchProcessing: %s"), 
               BatchSize, bEnableBatchProcessing ? TEXT("Enabled") : TEXT("Disabled"));
    }
}

void ARealGazeboBridgeManager::ConfigurePerformanceAndDebugSettings()
{
    if (!BridgeSubsystem.IsValid())
    {
        return;
    ...
    UE_LOG(LogRealGazeboBridgeManager, Verbose, TEXT("Performance settings configured - MaxActive: %d, UpdateRate: %.1f Hz"), 
           MaxActiveVehicles, UpdateFrequency);
}

bool ARealGazeboBridgeManager::ValidateConfiguration() const
{
    // Check port range
    if (ListenPort < 1024 || ListenPort > 65535)
    {
        UE_LOG(LogRealGazeboBridgeManager, Warning, TEXT("Invalid port number: %d. Should be between 1024-65535"), ListenPort);
        return false;
    }
    ...
    if (!VehicleDataTable)
    {
        UE_LOG(LogRealGazeboBridgeManager, Warning, TEXT("No Vehicle Data Table specified. Please assign a DataTable."));
        return false;
    }
    ...
    {
        UE_LOG(LogRealGazeboBridgeManager, Warning, TEXT("Invalid Vehicle Data Table format."));
        return false;
    }
...
void ARealGazeboBridgeManager::UpdateStatusDisplay()
{
    if (IsBridgeActive())
    {
...
bool ARealGazeboBridgeManager::IsValidVehicleDataTable(UDataTable* DataTable) const
{
    if (!DataTable)
    {
        return false;
    ...
    {
        UE_LOG(LogRealGazeboBridgeManager, Warning, TEXT("DataTable must use FBridgeVehicleConfigRow structure"));
        return false;
    }
    ...
    {
        UE_LOG(LogRealGazeboBridgeManager, Warning, TEXT("DataTable is empty - no vehicle configurations found"));
        return false;
    }
...
bool ARealGazeboBridgeManager::ValidatePoolConfiguration() const
{
    ...
    {
        UE_LOG(LogRealGazeboBridgeManager, Warning, TEXT("Invalid UnusedActorTimeout: %.1f. Should be between 10-300 seconds"), UnusedActorTimeout);
        return false;
    }
...
bool ARealGazeboBridgeManager::ValidateNetworkConfiguration() const
{
    ...
    {
        UE_LOG(LogRealGazeboBridgeManager, Warning, TEXT("Invalid BatchProcessingInterval: %.4f. Should be between 0.001-0.1 seconds"), BatchProcessingInterval);
        return false;
    }
...
bool ARealGazeboBridgeManager::ValidatePerformanceConfiguration() const
{
    ...
    {
        UE_LOG(LogRealGazeboBridgeManager, Warning, TEXT("Invalid UpdateFrequency: %.1f. Should be between 10-120 Hz"), UpdateFrequency);
        return false;
    }

    if (DefaultInterpolationSpeed < 1.0f || DefaultInterpolationSpeed > 100.0f)
    {
        UE_LOG(LogRealGazeboBridgeManager, Warning, TEXT("Invalid DefaultInterpolationSpeed: %.1f. Should be between 1-100"), DefaultInterpolationSpeed);
        return false;
    }
