...
for tool in curl tar make clang++ ar; do command -v "$tool" >/dev/null || { echo "error: missing required tool: $tool" >&2; exit 1; }; done
...
# Recent LIVE555 releases ship config.macos (not config.macosx). Detect the
# upstream platform config instead of hard-coding a historical name.
...
rm -rf "$DEST"
mkdir -p "$DEST/include/BasicUsageEnvironment" "$DEST/include/UsageEnvironment" "$DEST/include/groupsock" "$DEST/include/liveMedia"
...
for lib in "$DEST"/*.a; do
  ...
  if [[ "$(uname -m)" == arm64 ]] && ! lipo -info "$lib" 2>&1 | grep -q 'arm64'; then
